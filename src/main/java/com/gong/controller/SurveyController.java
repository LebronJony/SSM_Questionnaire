...
public class SurveyController {
    ...
    public String upload(Integer id, @RequestParam("file") MultipartFile multipartFile,HttpServletRequest request){
        ...
        String savaname = uuid + "_" + filename.substring(filename.lastIndexOf(File.separator) + 1);

        try {
            multipartFile.transferTo(new File(path,savaname));
            Survey survey = new Survey();
            survey.setId(id);
            survey.setBgimg(savaname);
